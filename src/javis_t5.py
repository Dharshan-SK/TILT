...
class TripleHeadTILTForTokenClassification(T5PreTrainedModel):
    def __init__(self, config, num_labels_0, num_labels_1, num_labels_2):
        ...
        # Initialize weights and apply final processing
        self.post_init()
        self.num_labels_0 = num_labels_0
        self.num_labels_1 = num_labels_1
        self.num_labels_2 = num_labels_2

        if num_labels_0 < 10:
            self.classifier_0 = nn.Linear(config.hidden_size, num_labels_0)
        else:
            config.num_labels = num_labels_0
            self.classifier_0 = LayoutLMv3ClassificationHead(config, pool_feature=False)
        
        if num_labels_1 < 10:
            self.classifier = nn.Linear(config.hidden_size, num_labels_1)
        else:
            # new_config = copy.deepcopy(config)
            config.num_labels = num_labels_1
            self.classifier = LayoutLMv3ClassificationHead(config, pool_feature=False)

        if num_labels_2 < 10:
            self.classifier_2 = nn.Linear(config.hidden_size, num_labels_2)
        else:
            config.num_labels = num_labels_2
            self.classifier_2 = LayoutLMv3ClassificationHead(config, pool_feature=False)
    ...
    def forward(
        ...
        hidden_states = outputs[0]
        hidden_states = self.dropout(hidden_states)
        logits_0 = self.classifier_0(hidden_states)
        logits_1 = self.classifier(hidden_states)
        logits_2 = self.classifier_2(hidden_states)

        loss_0, loss_1, loss_2 = None, None, None

        if labels_1 is not None:
            loss_fct = nn.CrossEntropyLoss()
            loss_0 = loss_fct(logits_0.view(-1, self.num_labels_0), labels_1.view(-1))

        if labels_2 is not None:
            loss_fct = nn.CrossEntropyLoss()
            loss_1 = loss_fct(logits_1.view(-1, self.num_labels_1), labels_2.view(-1))

        if labels_3 is not None:
            loss_fct = nn.CrossEntropyLoss()
            loss_2 = loss_fct(logits_2.view(-1, self.num_labels_2), labels_3.view(-1))

        
        return TokenClassifierOutput(
            loss= loss_0 + loss_1 + loss_2,
            logits=(logits_0, logits_1, logits_2, [loss_0, loss_1, loss_2]),
            hidden_states=outputs.hidden_states,
            attentions=outputs.attentions,
        )
        ...
        logits_0 = self.classifier_0(sequence_output)
        logits_1 = self.classifier(sequence_output)
